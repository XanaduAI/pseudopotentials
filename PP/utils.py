...
def compute_clean_cost(g):
    ...
    clean_cost = (3*g.eta*g.n_p, np.ceil(np.log2(np.ceil(np.pi*g.lambda_val/(2*g.error_qpe)))),\
        max([g.n_R+1, g.n_op, 35, g.n_b, g.n_k, g.n_Mloc, g.n_NL_prime, g.n_NL]), 1 , 2, 4, 2*g.eta+5, 8, \
            2*g.eta, g.n_Ltype+g.logn_a_max, 2,\
            g.n_Ltype+g.logn_a_max+5 , 4, 4, 2, 3*(g.n_p +1) , g.n_p, g.n_M_V,\
            3*g.n_p+2 , 2*g.n_p+1 , g.n_M_V+1, 1, 2, 3*(g.n_p+1), g.n_Ltype+ g.logn_a_max, 1,  \
            3+3+3+g.n_Ltype+9+4, 9, 2, 2*1) #2*1 for using n_AA 1 times in total
    clean_prep_temp_cost = max([5, 2*(g.n_Ltype + g.logn_a_max+1), g.beta_k + \
        (g.n_Ltype+4), 4, g.n_Ltype+2, g.beta_V+3*g.n_p, g.beta_loc+(3*g.n_p+g.n_Ltype)+g.n_Mloc])
    clean_temp_H_cost = max([5*g.n_p+1, 5*g.n_R-4]) + max([clean_prep_temp_cost, 3*g.beta_NL + \
        3*(g.n_p + g.n_Ltype+4), 3*g.n_p-1, 3*g.beta_NL_prime + 3*(g.n_p+g.n_Ltype+2) + 3])
    clean_temp_cost = max([clean_temp_H_cost, 2*g.n_eta + 9*g.n_p + g.n_Mloc + g.n_M_V + 35 + \
        2*(g.n_Ltype + g.logn_a_max) + 2*1])
    return sum(clean_cost) + clean_temp_cost
            
def compute_dirty_cost(g):
    return max([g.beta_k * g.n_k , g.beta_V * (g.n_M_V+1), g.beta_loc * (g.n_Mloc+1),\
