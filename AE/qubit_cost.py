...
def qubit_cost(n_p,n_M,n_R,n_T,beta, lambda_val, error_qpe,eta, lambda_zeta, n_B):
    ...
    clean_cost = (3*eta*n_p, np.ceil(np.log2(np.ceil(np.pi*lambda_val/(2*error_qpe)))),  np.max([n_R+1,n_T,n_B,35]), 1, \
    ...
    clean_prep_temp_cost = max([5,beta+3*n_p])
    ...
    logical_qubits = max([logical_clean_qubits,beta*(n_M+1)+(n_M+1)+3*n_p])
    print('clean logicals ', logical_clean_qubits)
    print('dirties ',beta*(n_M+1)+(n_M+1)+3*n_p)
